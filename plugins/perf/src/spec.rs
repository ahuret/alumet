...
fn resolve_task_attached(name: &str, rename: Option<&str>, modifiers: Modifiers) -> anyhow::Result<Vec<ParsedEvent>> {
    ...
    match placement {
        ...
        // A bare generic event: counted on every core PMU (both PMUs of a hybrid CPU).
...
/// One core PMU a bare generic event fans out to (see [`Placement::CoreGeneric`])
struct CoreTarget {
    /// The PMU's numeric type. `None` on a non-hybrid CPU.
    pmu_type: Option<u32>,
    binding: CoreBinding,
}

/// The core PMUs a bare generic hardware/cache event is counted on: every hybrid cluster
